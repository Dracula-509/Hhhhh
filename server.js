...
app.post('/admin/anime/:id/episodes', requireAdmin, (req, res) => {
  upload.single('video')(req, res, err => {
    const anime = db.prepare('SELECT * FROM animes WHERE id = ?').get(req.params.id);
    const episodes = db.prepare('SELECT * FROM episodes WHERE anime_id = ? ORDER BY number').all(req.params.id);
    if (err || !req.file)
      return res.render('admin-episodes', { anime, episodes, error: err ? err.message : 'Vidéo requise' });
    ...
    res.redirect('/admin/anime/' + req.params.id + '/episodes');
  });
});
